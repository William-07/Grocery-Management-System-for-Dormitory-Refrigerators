...
<h1 id="header">View stock records</h1>
    <!-- The main content  -->
    <main>
    <div id="content" align="center"> 
        ...
        // Query table
        $record_query = "SELECT grocery.* , users.Username As Username, stock_record.*
        FROM grocery
        INNER JOIN stock_record ON grocery.Item_ID = Stock_Record.Item_ID
        CROSS JOIN users ON grocery.User_ID = users.User_ID
        WHERE grocery.Item_Status = 0 ";

        $result = mysqli_query($conn, $record_query);

        // Display information in HTML
        echo "<table>";
        echo "<tr><th>Username</th><th>Items</th><th>Stock Date</th></tr>";
        ...
        while($row = mysqli_fetch_assoc($result)) {
            ...
            echo  "</td><td>" . $row["Username"] . "</td><td>" . $row["Item_Name"]  . "</td><td>" . $row["Stock_Date"] ."</td></tr>";
            $i++;
        }
        echo "<table>";
